...
export function generateCss(config: GeneratorConfig): string {
  ...
  const replacements: Record<string, string> = {
    ...
    listenerBorder: hasBorder
      ? '3px solid var(--listener-comment-border)'
      : 'none',
    memberBorder: hasBorder
      ? '3px solid var(--member-comment-border)'
      : 'none',
    pointerContent: hasBorder ? '""' : 'none',
    autoMarginSide: isLeft ? 'margin-right' : 'margin-left',
    pointerOuterPosition: isLeft
      ? 'left: -3px; right: auto;'
      : 'left: auto; right: -3px;',
    pointerOuterTransform: isLeft
      ? 'rotate(-20deg) skew(20deg, 20deg)'
      : 'rotate(-70deg) skew(20deg, 20deg)',
    pointerInnerPosition: isLeft
      ? 'left: 1px; right: auto;'
      : 'left: auto; right: 1px;',
    pointerInnerTransform: isLeft
      ? 'rotate(-20deg) skew(20deg, 20deg)'
